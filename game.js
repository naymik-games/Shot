...
class playGame extends Phaser.Scene {
  ...
  create() {
    ...
    this.cameras.main.setBounds(-2100, -2100, 4200, 4200);
    this.physics.world.setBounds(-2100, -2100, 4200, 4200);
    this.cameras.main.setZoom(.5)
    /* this.add.image(-1024, -1024, 'bg').setOrigin(0);
    this.add.image(0, -1024, 'bg').setOrigin(0);
    this.add.image(-1024, 0, 'bg').setOrigin(0); */
    var pic = this.add.image(0, 0, 'sky').setOrigin(.5).setScale(3);
    var bounds = this.cameras.main.getBounds();
    var gSize = 40
    var g1 = this.add.grid(0, 0, bounds.width, bounds.height, gSize, gSize, 0x333333, 0, 0xcccccc, .5).setOrigin(.5);
    this.target = this.add.image(13 * gSize, 0, 'target')
    this.cursors = this.input.keyboard.createCursorKeys();
    this.spot = this.add.image(0, 0, 'particle').setTint(0xff0000).setAlpha(0)

    this.player = this.physics.add.image(0, 0, 'scope').setScale(.5).setAlpha(0)
    ...
    this.text = this.add.text(32, 62).setScrollFactor(0).setFontSize(62).setColor('#000000');
  ...
  toggleScope() {
    if (this.cameras.main.zoom == .5) {
      ...
      this.cameras.main.setBounds(-2100 * 3, -1050 * 3, 4200 * 3, 4200 * 3);
      this.physics.world.setBounds(-2100 * 3, -1050 * 3, 4200 * 3, 4200 * 3);
    } else {
      this.cameras.main.setZoom(.5)
